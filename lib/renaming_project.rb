class RenamingProject
  def self.perform(new_name)
    Dir.glob("#{Rails.root}/**/*.*").each do |filepath|
      ...
      file_content = file_content.gsub("TheUltimateScienceFictionAwardWinningBooksInternetDatabase", new_name)
      file_content = file_content.gsub("theultimatesciencefictionawardwinningbooksinternetdatabase", new_name.downcase)
