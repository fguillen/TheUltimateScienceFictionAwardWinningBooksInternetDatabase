class Notifier < ActionMailer::Base
  ...
  def admin_user_reset_password(admin_user)
    ...
    mail(
      to: admin_user.email,
      subject: "[TheUltimateScienceFictionAwardWinningBooksInternetDatabase] Password reset"
    )
  end
  ...
  def simple_test_email(subject, to)
    mail(
      to: to,
      subject: "[TheUltimateScienceFictionAwardWinningBooksInternetDatabase] #{subject}"
    )
  end
end
